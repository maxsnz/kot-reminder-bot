...
src/bot/parser/
  ...
  tokenizer.ts                — разбивка текста на токены (функции экспортированы для переиспользования)
  parseSnoozeInput.ts         — парсер кастомного времени снуза (переиспользует tokenizer)
...
## Переиспользование токенайзера: кастомный снуз

`parseSnoozeInput.ts` использует те же функции токенайзера (`normalize`, `extractRelative`, `extractDate`, `extractCalendarDate`, `extractTime`) для парсинга произвольного времени снуза, введённого пользователем после нажатия кнопки "...".

Поддерживаемые форматы ввода:
- "через 2 часа", "через 30 минут" — relative offset
- "5 часов", "2 часа" — bare duration (без "через")
- "завтра", "послезавтра" — смещение +24h/+48h
- "завтра в 10 утра", "послезавтра в 14" — следующий день в указанное время
- "в пятницу в 18:00" — следующий такой день недели
- "23 июня в 10" — конкретная дата
- "в 14:00", "в 9 утра" — сегодня в указанное время

Возвращает `{ ok: true, runAt: Date }` или `{ ok: false, reason: "no_match" | "time_in_past" | "ambiguous_time" }`.
